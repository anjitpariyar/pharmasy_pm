function edit(action) {
  ...
  if (!action) document.getElementById("admin_acknowledgement").innerHTML = "";
}
...
function updateAdminDetails() {
  ...
  if (!validateName(pharmacy_name.value, "pharmacy_name_error"))
    ...
    if (validatePassword(password)) {
      ...
      xhttp.open(
        "GET",
        "php/validateCredentials.php?action=update_admin_info&pharmacy_name=" +
          pharmacy_name.value +
          "&address=" +
          address.value +
          "&email=" +
...
function validatePassword(password) {
  ...
  xhttp.open(
    "GET",
    "php/validateCredentials.php?action=validate_password&password=" + password,
    false
  );
  xhttp.send();
  if (xhttp.responseText == "true") return true;
  return false;
}
...
function checkAdminPassword(password, error) {
  ...
  if (validatePassword(password)) {
    ...
    return true;
  } else document.getElementById(error).innerHTML = "Wrong Password!!!";
  return false;
}
...
function changePassword() {
  ...
  if (!checkAdminPassword(old_password.value, "old_password_error"))
    ...
    xhttp.open(
      "GET",
      "php/validateCredentials.php?action=change_password&password=" +
        password.value,
      false
    );
    xhttp.send();
