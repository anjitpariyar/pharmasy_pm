...
function addRow() {
  ...
  xhttp.open(
    "GET",
    "php/add_new_purchase.php?action=add_row&row_id=" +
      id.value +
      "&row_number=" +
      addRow.counter,
    true
...
function isSupplier(name) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_purchase.php?action=is_supplier&name=" + name,
    false
  );
  xhttp.send();
...
function checkInvoice(invoice_number, error) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_purchase.php?action=is_invoice&invoice_number=" +
      invoice_number,
    false
  );
  xhttp.send();
...
function isNewMedicine(name, packing) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_purchase.php?action=is_new_medicine&name=" +
      name +
      "&packing=" +
      packing,
    false
...
function getAmount(row_number) {
  ...
  for (var i = 1; i < row_count; i++) {
    amount = Number.parseFloat(
      medicine_info[i].children[0].children[7].children[0].children[0].value
    );
    total += amount;
  }
...
function addPurchase() {
  ...
  if (!notNull(suppliers_name.value, "supplier_name_error"))
    ...
    document.getElementById("supplier_name_error").innerHTML =
    ...
    invoice_number.focus();
  else if (!checkDate(invoice_date.value, "date_error")) invoice_date.focus();
  else {
    var parent = document.getElementById("purchase_medicine_list_div");
    var row_count = parent.childElementCount;
    var medicine_info = parent.children;
    ...
    for (var i = 1; i < row_count; i++) {
      ...
      if (!notNull(medicine_name.value, medicine_name_error.getAttribute("id")))
        ...
        !checkExpiry(expiry_date.value, expiry_date_error.getAttribute("id")) ||
        checkExpiry(expiry_date.value, expiry_date_error.getAttribute("id")) ==
          -1
      )
        expiry_date.focus();
      else if (
        !checkQuantity(quantity.value, quantity_error.getAttribute("id"))
      )
        quantity.focus();
        ...
        isNewMedicine(medicine_name.value, packing.value) == "true" &&
        generic_name.value == ""
      ) {
        ...
        medicineStockRow[i - 1] = new MedicineStock(
          medicine_name.value,
          batch_id.value,
          expiry_date.value,
          quantity.value,
        ...
        newMedicine[i - 1] = new NewMedicine(
          medicine_name.value,
          packing.value,
          generic_name.value,
    ...
    // bsdk
    // console.log("bsdk starting ma ta aaije");
    ...
    for (var i = 0; i < row_count - 1; i++) {
      ...
      if (isNewMedicine(newMedicine[i].name, newMedicine[i].packing) == "true")
        addNewMedicine(
          newMedicine[i].name,
          newMedicine[i].packing,
          newMedicine[i].generic_name,
          newMedicine[i].supplier_name
        );
      addMedicineStock(
        medicineStockRow[i].name,
        medicineStockRow[i].batch_id,
        medicineStockRow[i].expiry_date,
        medicineStockRow[i].quantity,
        medicineStockRow[i].mrp,
        medicineStockRow[i].rate,
...
function addMedicineStock(
  ...
  var xhttp = new XMLHttpRequest();
  console.log("bsdk add medicine stock");
  xhttp.onreadystatechange = function () {
    if ((xhttp.readyState = 4 && xhttp.status == 200)) {
      xhttp.responseText;
      console.log(" console.log(xhttp.responseText);", xhttp.responseText);
    } else {
      console.log("sorrybro");
    }
  };
  xhttp.open(
    "GET",
    "php/add_new_purchase.php?action=add_stock&name=" +
      name +
      "&batch_id=" +
      batch_id +
      "&expiry_date=" +
...
function addNewPurchase(
  ...
  xhttp.onreadystatechange = function () {
    if ((xhttp.readyState = 4 && xhttp.status == 200))
      document.getElementById("purchase_acknowledgement").innerHTML =
        xhttp.responseText;
  };
  xhttp.open(
    "GET",
    "php/add_new_purchase.php?action=add_new_purchase&suppliers_name=" +
      suppliers_name +
      "&invoice_number=" +
      invoice_number +
      "&payment_type=" +
